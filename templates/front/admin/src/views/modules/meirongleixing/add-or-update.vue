<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="美容类型" prop="meirongleixing">
					<el-input  v-model="ruleForm.meirongleixing" placeholder="美容类型" clearable  :readonly="ro.meirongleixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="美容类型" prop="meirongleixing">
					<el-input v-model="ruleForm.meirongleixing" placeholder="美容类型" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				meirongleixing: [
					{ required: true, message: '美容类型不能为空', trigger: 'blur' },
				],
			}
		};
	},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `meirongleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.meirongleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
