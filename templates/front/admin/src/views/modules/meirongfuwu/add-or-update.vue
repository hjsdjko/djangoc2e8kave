	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="美容类型" prop="meirongleixing">
					<el-select :disabled="ro.meirongleixing" v-model="ruleForm.meirongleixing" placeholder="请选择美容类型" >
						<el-option
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="美容类型" prop="meirongleixing">
					<el-input v-model="ruleForm.meirongleixing"
						placeholder="美容类型" readonly></el-input>
				</el-form-item>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `meirongfuwu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.meirongfuwuCrossAddOrUpdateFlag = false;
						 this.parent.search();
