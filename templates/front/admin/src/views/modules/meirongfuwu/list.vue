		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"80%","margin":"0 0 20px","alignItems":"flex-start","flexDirection":"column","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #654B3C","width":"auto","overflow":"hidden","borderRadius":"20px","display":"inline-block"}' >
					...
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}' class="select" label="美容类型" prop="meirongleixing">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">美容类型</label>
						<el-select clearable v-model="searchForm.meirongleixing" placeholder="请选择美容类型" >
							<el-option v-for="(item,index) in meirongleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('meirongfuwu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						创建
					</el-button>
					<el-button class="del" v-if="isAuth('meirongfuwu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px","boxShadow":""}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('meirongfuwu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"14px","color":"#965F40","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('meirongfuwu','预约')" @click="fuwuyuyueCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-qita20" :style='{"margin":"0 2px","fontSize":"14px","color":"#965F40","height":"40px"}'></span>
								预约
							</el-button>
							<el-button class="edit" v-if=" isAuth('meirongfuwu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai13" :style='{"margin":"0 2px","fontSize":"14px","color":"#965F40","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('meirongfuwu','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"14px","color":"#965F40","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('meirongfuwu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import fuwuyuyueCrossAddOrUpdate from "../fuwuyuyue/add-or-update";
	export default {
		...
		methods: {
			...
			fuwuyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fuwuyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','meirongfuwu');
    ...
    init () {
          this.$http({
            url: `option/meirongleixing/meirongleixing`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.meirongleixingOptions = data.data;
            } else {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.meirongleixing!='' && this.searchForm.meirongleixing!=undefined){
            params['meirongleixing'] = this.searchForm.meirongleixing
          }
	...
	this.$router.push({path:'/discussmeirongfuwu',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	meirongfuwustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'meirongfuwu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
