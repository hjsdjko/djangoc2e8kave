...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"left","top":"50px","left":"10%","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"calc(80% - 340px)","position":"absolute","borderStyle":"solid","height":"auto","order":"1"}'>
      ...
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>美容类型：</div>
        <el-select v-model="formSearch.meirongleixing" placeholder="请选择美容类型" :clearable="true">
          <el-option v-for="(item, index) in meirongleixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	  ...
	  <el-button v-if="btnAuth('meirongfuwu','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"0px","background":"#c5c5c5","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/meirongfuwuAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/meirongleixing/meirongleixing').then(res => {
			if (res.data.code == 0) {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('meirongleixing/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'meirongleixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.meirongleixing != '') searchWhere.meirongleixing = this.formSearch.meirongleixing;
        ...
        if (this.curFenlei != '全部') searchWhere.meirongleixing = this.curFenlei;
        this.$http.get(`meirongfuwu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/meirongfuwuDetail', query: params});
      },
