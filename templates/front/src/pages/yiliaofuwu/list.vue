...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"left","top":"50px","left":"10%","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"calc(80% - 340px)","position":"absolute","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>项目名称：</div>
        <el-input v-model="formSearch.xiangmumingcheng" placeholder="项目名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>医疗分类：</div>
        <el-select v-model="formSearch.yiliaofenlei" placeholder="请选择医疗分类" :clearable="true">
          <el-option v-for="(item, index) in yiliaofenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>项目价格：</div>
        <el-input v-model="formSearch.xiangmujiagestart" placeholder="最小项目价格" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
        <el-input v-model="formSearch.xiangmujiageend" placeholder="最大项目价格" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('yiliaofuwu','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"0px","background":"#c5c5c5","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/yiliaofuwuAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"20px 0 0 2%","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid #f6f6f6","width":"100%","padding":"0px","margin":"0 0px 0 0","overflow":"hidden","height":"320px"}' class="img-box">
				<img @click.stop="imgPreView(item.xiangmutupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.xiangmutupian && item.xiangmutupian.substr(0,4)=='http'" :src="item.xiangmutupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.xiangmutupian?item.xiangmutupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.xiangmutupian?item.xiangmutupian.split(',')[0]:'')" class="image" />
			</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/yiliaofenlei/yiliaofenlei').then(res => {
			if (res.data.code == 0) {
				this.yiliaofenleiOptions = res.data.data;
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('yiliaofenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'yiliaofenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.xiangmumingcheng != '') searchWhere.xiangmumingcheng = '%' + this.formSearch.xiangmumingcheng + '%';
        if (this.formSearch.yiliaofenlei != '') searchWhere.yiliaofenlei = this.formSearch.yiliaofenlei;
        if(this.formSearch.xiangmujiagestart!='' && this.formSearch.xiangmujiagestart!=undefined ){
          searchWhere.xiangmujiagestart = this.formSearch.xiangmujiagestart
        }
        if(this.formSearch.xiangmujiageend!='' && this.formSearch.xiangmujiageend!=undefined){
          searchWhere.xiangmujiageend = this.formSearch.xiangmujiageend
        }
        if (this.curFenlei != '全部') searchWhere.yiliaofenlei = this.curFenlei;
        this.$http.get(`yiliaofuwu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/yiliaofuwuDetail', query: params});
      },
