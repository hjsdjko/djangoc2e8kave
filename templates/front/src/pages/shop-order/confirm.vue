                ...
                this.$http.get(`${item.tablename}/info/${item.goodid}`).then(res => {
                    ...
                    this.$http.post(`${item.tablename}/update`, data).then(res => {
                        ...
                        this.$http.post('orders/add', order).then(res => {
                            ...
                            // 如果该商品存在积分
                            if (data.jf) {
                                this.user.jf = (Number(this.user.jf) + Number(this.totalPrice)).toFixed(2)
                            }
                            this.user.money = (Number(this.user.money) - Number(this.totalPrice)).toFixed(2)
                            // 更新用户余额
                ...
                this.$http.get(`${item.tablename}/info/${item.goodid}`).then(res => {
                    ...
                    this.$http.post(`${item.tablename}/update`, data).then(res => {
                        ...
                        this.$http.post('orders/add', order).then(res => {
                            // 减少余额，更新订单状态
                            // 判断余额是否充足
                            if (parseInt(this.user.jf) < parseInt(this.totalPrice)) {
                                this.$message({
                                    message: '积分不足',
                                    type: 'error',
                                    duration: 1500
